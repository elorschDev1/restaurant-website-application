...
const SearchBar = () => {
   ...
   let [retrievedPriceCategories,setRetrievedPriceCategories]=useState([]);
    ...
    let priceCategorySearch=async()=>{
            let formData=new FormData();
            formData.append("lowerPriceLimit",lowerPriceLimit);
            formData.append("upperPriceLimit",upperPriceLimit);
            formData.append("selectedCategory",selectedCategory);
            let res=await fetch("http://localhost/restaurant_backend-app/pricecategorysearch.php",{
                method:"POST",
                body:formData
            });
            let data=await res.json();
            setRetrievedPriceCategories([...data]);
    }
       ...
       {userMeals.map((userMeal,index)=>(
            <div key={index}>
                ...
                <Row className='p-3 m-4'>
                    ...
                    <div className={userMeal.meal} id={userMeal.price}><AddToCart/></div> 
                    </Card.Body>
                  </Card>     
                    </Col>
                </Row>
        ...
        {retrievedPrices.map((retrievedPrice,index)=>(
            <div key={index}>
                ...
                <Row className='p-3 m-4'>
                  ...
                  <div className={retrievedPrice.meal} id={retrievedPrice.price}><AddToCart/></div> 
                    </Card.Body>
                  </Card>     
                    </Col>
                </Row>
        ...
        {retrievedCategories.map((retrievedCategory,index)=>(
            <div key={index}>
                ...
                <Row className='p-3 m-4'>
                    ...
                    <div className={retrievedCategory.meal} id={retrievedCategory.price}><AddToCart/></div> 
                    </Card.Body>
                  </Card>     
                    </Col>
                </Row>
   ...
   if(selectedCategory!==""&&selectedPrice!==""&&buttonClicked===false&&searchedMeal===""){
    ...
    <div className="d-flex flex-row p-3 m-3">
       ...
       <div>
        {retrievedPriceCategories.map((retrievedPriceCategory,index)=>(
            <div key={index}>
                  <div key={index}>
                <Row className='p-3 m-4'>
                    <Col className="d-flex certainCol mx-auto" md={6}>
                       <Card className="w-100 card-hover">
                    <Card.Img src={retrievedPriceCategory.image} variant='top' className='custom-img'/>
                    <Card.Body className='d-flex flex-column'>
                      <Card.Title>{retrievedPriceCategory.meal}</Card.Title>
                      <Card.Text>{retrievedPriceCategory.description}</Card.Text>
                      <Card.Text className='price'>Ksh.{retrievedPriceCategory.price}</Card.Text>
                    <div className={retrievedPriceCategory.meal} id={retrievedPriceCategory.price}><AddToCart/></div> 
                    </Card.Body>
                  </Card>     
                    </Col>
                </Row>
